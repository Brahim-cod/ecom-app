...
import ShowMainCommentModal from "./ShowMainCommentModal";
...
const OrderItemCard = ({ item, handleDelete, username }) => {
  ...
  const handleSetComment = (uuid) => {
    console.log(uuid);
    const previousItem = JSON.parse(localStorage.getItem("item"));
    if (previousItem) {
      const IsThisItem = previousItem?.find((pIt) => pIt.uuid === uuid);
      setComment2(IsThisItem.comment);
    }
  };
  ...
  const handleAddComment = (id, uuid) => {
    ...
    if (previousItem) {
      ...
      if (IsThisItem) {
        const OpItem = IsThisItem?.options.find((oIt) => oIt.id === id);
        console.log(OpItem);
        OpItem.comment = inputValue;
        const newOPItem = IsThisItem?.options.filter((oIt) => oIt.id !== id);
        ...
        <div className='flex w-full items-center justify-between   gap-[14px]'>
          ...
          <div>
            ...
            <div className='flex w-full items-center justify-between'>
              <p className='price mr-[53px] lg:mr-[60px]'>{item?.price} Dhs</p>
              <div className='input-group mr-[10px] '>
                <button onClick={() => setDecrease(item.uuid)}>
                  <svg
                    xmlns='http://www.w3.org/2000/svg'
                    width='12'
                    height='3'
                ...
                <p>{value}</p>
                <button onClick={() => setIncrease(item.uuid)}>
                  <svg
                    xmlns='http://www.w3.org/2000/svg'
                    width='11'
                    height='11'
              ...
              <dialog id='my_modal_1' className='modal'>
                <div className='modal-box'>
                  ...
                  <p className='px-[20px] description'>{comment2}</p>
                  <form method='dialog'>
